...
def output(d0_last_state, d1_last_state, x_input):
    return int(d0_last_state == 1 and d1_last_state == 1 and x_input == 1)
...
for i in range(10):
    if i==0:
        ...
        output_z.append(output(output_q0[i-1],output_q1[i-1],input_x[i]))
    elif i==5:
        output_q0.append(0)
        output_q1.append(0)
        output_z.append(d1(output_q0[i-1],output_q1[i-1],input_x[i]))
